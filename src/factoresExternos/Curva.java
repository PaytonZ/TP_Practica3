...
public class Curva implements InterfaceEjecuta {
	...
	{
	    java.util.Iterator<Entry<Double, Double>> it = mapaCurvas.entrySet().iterator();
		
	    while(it.hasNext())
	    {
		Entry<Double, Double> elemento = it.next();
		double espacio_recorrido = vectorBicis.get(i).getEspacioRecorrido();
		//comprobamos que llevamos una velocidad adecuada a la curva en su 
		//distancia de frenado, para asi reducirla y conseguir una velocidad
		//adecuada
		if( espacio_recorrido >= elemento.getKey() - Constantes.DISTANCIA_FRENADO 
